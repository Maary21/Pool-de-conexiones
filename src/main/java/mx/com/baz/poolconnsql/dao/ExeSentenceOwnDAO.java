...
public class ExeSentenceOwnDAO {
	...
	public Boolean saveLecturaTran(TcLecturaTrans data){
		try {
			...
			SqlParameterSource in = new MapSqlParameterSource().addValue("C_FCPARSEO", data.getFcParseo())
					...
					.addValue("C_FIMONTO", data.getFiMonto())
					.addValue("C_FCCONCILIAUNION", data.getFcConciliaUnion());
			jdbcCall.execute(in);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Boolean saveLecturaTranBatch(List<TcLecturaTrans> dataList) {
		try {
			JdbcTemplate jdbcTemplate = new JdbcTemplate(DataSourceOwnConfig.getDataSource());
			jdbcTemplate.batchUpdate(" INSERT INTO TCLECTURATRAN (PKIDTRANSACCION,FCPARSEO,FCRESULTADO,FCNOMBRE,FIESTATUS,FKIDCONCILIACION,FCFASE,FCUSRMODIF,FDFECMODIF,FDFECALTA, FIORIGENINFO, FIMONTO, FCCONCILIAUNION) "
					+ " SELECT id_tran_seq.NEXTVAL,?,?,?,?,?,FN_AVANCE_INIT(?),?,?,?,?,?,? FROM DUAL ",dataList, 100, (PreparedStatement ps, TcLecturaTrans dto) -> {
						...
						ps.setDouble(11, dto.getFiMonto());
						ps.setString(12, dto.getFcConciliaUnion());
					});	
			return true;
		} catch (Exception e) {
